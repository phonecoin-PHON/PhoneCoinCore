...
#if defined(NDEBUG)
#error "PhoneCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uphonecoin) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // premine
      ...
      nSubsidy = 0.2 * COIN;
    } else if (nHeight > 6000 && nHeight <= 25000) {
      nSubsidy = 10 * COIN;
...
{
    RenameThread("phonecoin-scriptch");
